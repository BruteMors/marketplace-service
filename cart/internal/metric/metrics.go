	...
	metrics = &Metrics{
		requestCounter: promauto.NewCounter(
			prometheus.CounterOpts{
				...
				Help:      "Количество запросов к HTTP серверу",
			},
		),
		responseCounter: promauto.NewCounterVec(
			prometheus.CounterOpts{
				...
				Help:      "Количество ответов от HTTP сервера",
			},
		...
		histogramResponseTime: promauto.NewHistogramVec(
			prometheus.HistogramOpts{
				...
				Help:      "Время ответа от HTTP сервера",
		...
		externalRequestCounter: promauto.NewCounterVec(
			prometheus.CounterOpts{
				Namespace: namespace,
				Subsystem: "external",
				Name:      appName + "_external_requests_total",
				Help:      "Количество запросов к внешним ресурсам",
			},
			[]string{"status", "url"},
		),
		externalHistogramResponseTime: promauto.NewHistogramVec(
			prometheus.HistogramOpts{
				Namespace: namespace,
				Subsystem: "external",
				Name:      appName + "_external_response_time_seconds",
				Help:      "Время ответа от внешних ресурсов",
				Buckets:   prometheus.ExponentialBuckets(0.0001, 2, 16),
			},
			[]string{"status", "url"},
		),
		...
		inMemoryObjectCount: promauto.NewGauge(
			prometheus.GaugeOpts{
				Namespace: namespace,
				Subsystem: "repository",
				Name:      appName + "_in_memory_object_count",
				Help:      "Количество объектов в in-memory репозитории",
			},
		),
	}
...
func IncExternalRequestCounter(status, url string) {
	metrics.externalRequestCounter.WithLabelValues(status, url).Inc()
}

func ObserveExternalResponseTime(status, url string, time float64) {
	metrics.externalHistogramResponseTime.WithLabelValues(status, url).Observe(time)
}
...
func AddInMemoryObjectCount(num float64) {
	metrics.inMemoryObjectCount.Add(num)
}

func SubInMemoryObjectCount(num float64) {
	metrics.inMemoryObjectCount.Sub(num)
}
